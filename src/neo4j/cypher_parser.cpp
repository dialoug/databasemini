#include "minidb/neo4j/cypher_parser.hpp"

#include <algorithm>
#include <charconv>
#include <cctype>
#include <set>
#include <stdexcept>
#include <string_view>
#include <utility>
#include <vector>

namespace minidb::neo4j {

std::string ReturnItem::display_name() const {
    return property.has_value() ? variable + "." + *property : variable;
}

namespace {

enum class TokenKind { word, number, string, symbol, arrow, end };

struct Token {
    TokenKind kind;
    std::string text;
};

class Lexer {
public:
    explicit Lexer(std::string_view input) : input_(input) {
        if (input_.size() >= 3 &&
            static_cast<unsigned char>(input_[0]) == 0xEF &&
            static_cast<unsigned char>(input_[1]) == 0xBB &&
            static_cast<unsigned char>(input_[2]) == 0xBF) {
            position_ = 3;
        }
    }

    std::vector<Token> scan() {
        std::vector<Token> tokens;
        while (position_ < input_.size()) {
            const auto character = input_[position_];
            if (std::isspace(static_cast<unsigned char>(character))) {
                ++position_;
            } else if (std::isalpha(static_cast<unsigned char>(character)) ||
                       character == '_') {
                tokens.push_back(scan_word());
            } else if (std::isdigit(static_cast<unsigned char>(character)) ||
                       (character == '-' && position_ + 1 < input_.size() &&
                        std::isdigit(static_cast<unsigned char>(input_[position_ + 1])))) {
                tokens.push_back(scan_number());
            } else if (character == '\'' || character == '"') {
                tokens.push_back(scan_string(character));
            } else if (character == '-' && position_ + 1 < input_.size() &&
                       input_[position_ + 1] == '>') {
                tokens.push_back(Token{TokenKind::arrow, "->"});
                position_ += 2;
            } else if (std::string_view("(){}[],:;.-").find(character) !=
                       std::string_view::npos) {
                tokens.push_back(Token{TokenKind::symbol, std::string(1, character)});
                ++position_;
            } else {
                throw std::runtime_error("无法识别的 Cypher 字符: " +
                                         std::string(1, character));
            }
        }
        tokens.push_back(Token{TokenKind::end, {}});
        return tokens;
    }

private:
    Token scan_word() {
        const auto start = position_;
        while (position_ < input_.size()) {
            const auto character = input_[position_];
            if (!std::isalnum(static_cast<unsigned char>(character)) &&
                character != '_') {
                break;
            }
            ++position_;
        }
        return Token{TokenKind::word,
                     std::string(input_.substr(start, position_ - start))};
    }

    Token scan_number() {
        const auto start = position_;
        if (input_[position_] == '-') {
            ++position_;
        }
        while (position_ < input_.size() &&
               std::isdigit(static_cast<unsigned char>(input_[position_]))) {
            ++position_;
        }
        return Token{TokenKind::number,
                     std::string(input_.substr(start, position_ - start))};
    }

    Token scan_string(char quote) {
        ++position_;
        std::string value;
        while (position_ < input_.size()) {
            const auto character = input_[position_++];
            if (character == quote) {
                return Token{TokenKind::string, std::move(value)};
            }
            if (character != '\\') {
                value.push_back(character);
                continue;
            }
            if (position_ >= input_.size()) {
                throw std::runtime_error("字符串末尾存在不完整的转义");
            }
            const auto escaped = input_[position_++];
            switch (escaped) {
                case 'n': value.push_back('\n'); break;
                case 'r': value.push_back('\r'); break;
                case 't': value.push_back('\t'); break;
                case '\\': value.push_back('\\'); break;
                case '\'': value.push_back('\''); break;
                case '"': value.push_back('"'); break;
                default:
                    throw std::runtime_error("不支持的字符串转义: \\" +
                                             std::string(1, escaped));
            }
        }
        throw std::runtime_error("字符串缺少结束引号");
    }

    std::string_view input_;
    std::size_t position_{0};
};

bool equals_keyword(const std::string& value, std::string_view keyword) {
    if (value.size() != keyword.size()) {
        return false;
    }
    for (std::size_t index = 0; index < value.size(); ++index) {
        if (std::toupper(static_cast<unsigned char>(value[index])) != keyword[index]) {
            return false;
        }
    }
    return true;
}

class Parser {
public:
    explicit Parser(std::vector<Token> tokens) : tokens_(std::move(tokens)) {}

    Statement parse() {
        if (match_keyword("CREATE")) {
            auto pattern = parse_graph_pattern();
            if (pattern.relationship.has_value() &&
                !pattern.relationship->type.has_value()) {
                throw std::runtime_error("CREATE 关系必须指定类型，例如 [:KNOWS]");
            }
            finish();
            return CreateStatement{std::move(pattern)};
        }
        if (match_keyword("MATCH")) {
            auto pattern = parse_graph_pattern();
            expect_keyword("RETURN");
            auto returns = parse_return_items();
            validate_match_variables(pattern, returns);
            finish();
            return MatchStatement{std::move(pattern), std::move(returns)};
        }
        if (match_keyword("BEGIN")) {
            match_keyword("TRANSACTION");
            finish();
            return BeginStatement{};
        }
        if (match_keyword("COMMIT")) {
            finish();
            return CommitStatement{};
        }
        if (match_keyword("ROLLBACK")) {
            finish();
            return RollbackStatement{};
        }
        if (match_keyword("SHOW")) {
            if (match_keyword("LABELS")) {
                finish();
                return ShowLabelsStatement{};
            }
            if (match_keyword("RELATIONSHIP")) {
                expect_keyword("TYPES");
                finish();
                return ShowRelationshipTypesStatement{};
            }
            expect_keyword("ARCHITECTURE");
            finish();
            return ShowArchitectureStatement{};
        }
        if (match_keyword("HELP")) {
            finish();
            return HelpStatement{};
        }
        if (match_keyword("QUIT") || match_keyword("EXIT")) {
            finish();
            return QuitStatement{};
        }
        throw std::runtime_error("不支持的 Cypher 命令；输入 HELP 查看支持的语法");
    }

private:
    GraphPattern parse_graph_pattern() {
        GraphPattern pattern{parse_node_pattern(), std::nullopt, std::nullopt};
        if (!match_symbol("-")) {
            return pattern;
        }
        expect_symbol("[");
        pattern.relationship = parse_relationship_pattern();
        expect_symbol("]");
        expect_arrow();
        pattern.right = parse_node_pattern();
        return pattern;
    }

    NodePattern parse_node_pattern() {
        expect_symbol("(");
        NodePattern pattern;
        if (peek().kind == TokenKind::word && !is_property_keyword(peek().text)) {
            pattern.variable = advance().text;
        }
        while (match_symbol(":")) {
            const auto label = expect_word("节点标签");
            if (std::find(pattern.labels.begin(), pattern.labels.end(), label) !=
                pattern.labels.end()) {
                throw std::runtime_error("重复节点标签: " + label);
            }
            pattern.labels.push_back(label);
        }
        if (check_symbol("{")) {
            pattern.properties = parse_properties();
        }
        expect_symbol(")");
        return pattern;
    }

    RelationshipPattern parse_relationship_pattern() {
        RelationshipPattern pattern;
        if (peek().kind == TokenKind::word && !is_property_keyword(peek().text)) {
            pattern.variable = advance().text;
        }
        if (match_symbol(":")) {
            pattern.type = expect_word("关系类型");
        }
        if (check_symbol("{")) {
            pattern.properties = parse_properties();
        }
        return pattern;
    }

    PropertyMap parse_properties() {
        expect_symbol("{");
        PropertyMap properties;
        if (match_symbol("}")) {
            return properties;
        }
        while (true) {
            std::string key;
            if (peek().kind == TokenKind::word || peek().kind == TokenKind::string) {
                key = advance().text;
            } else {
                throw std::runtime_error("属性名必须是标识符或字符串");
            }
            expect_symbol(":");
            if (!properties.emplace(key, parse_property_value()).second) {
                throw std::runtime_error("重复属性: " + key);
            }
            if (!match_symbol(",")) {
                break;
            }
        }
        expect_symbol("}");
        return properties;
    }

    PropertyValue parse_property_value() {
        if (peek().kind == TokenKind::string) {
            return advance().text;
        }
        if (peek().kind == TokenKind::number) {
            const auto text = advance().text;
            std::int64_t value = 0;
            const auto parsed = std::from_chars(text.data(), text.data() + text.size(), value);
            if (parsed.ec != std::errc{} || parsed.ptr != text.data() + text.size()) {
                throw std::runtime_error("整数超出 int64 范围: " + text);
            }
            return value;
        }
        if (peek().kind == TokenKind::word) {
            const auto word = advance().text;
            if (equals_keyword(word, "TRUE")) {
                return true;
            }
            if (equals_keyword(word, "FALSE")) {
                return false;
            }
            if (equals_keyword(word, "NULL")) {
                return nullptr;
            }
            throw std::runtime_error("属性字符串需要使用单引号或双引号: " + word);
        }
        throw std::runtime_error("期望属性值");
    }

    std::vector<ReturnItem> parse_return_items() {
        std::vector<ReturnItem> items;
        while (true) {
            ReturnItem item{expect_word("RETURN 变量"), std::nullopt};
            if (match_symbol(".")) {
                item.property = expect_word("属性名");
            }
            items.push_back(std::move(item));
            if (!match_symbol(",")) {
                break;
            }
        }
        return items;
    }

    static void validate_match_variables(const GraphPattern& pattern,
                                         const std::vector<ReturnItem>& returns) {
        std::set<std::string> variables;
        const auto add = [&](const std::optional<std::string>& variable) {
            if (variable.has_value() && !variables.insert(*variable).second) {
                throw std::runtime_error("MATCH 模式中的变量重复: " + *variable);
            }
        };
        add(pattern.left.variable);
        if (pattern.relationship.has_value()) {
            add(pattern.relationship->variable);
        }
        if (pattern.right.has_value()) {
            add(pattern.right->variable);
        }
        for (const auto& item : returns) {
            if (!variables.contains(item.variable)) {
                throw std::runtime_error("RETURN 引用了未绑定变量: " + item.variable);
            }
        }
    }

    static bool is_property_keyword(const std::string& word) {
        return equals_keyword(word, "TRUE") || equals_keyword(word, "FALSE") ||
               equals_keyword(word, "NULL");
    }

    const Token& peek() const { return tokens_[position_]; }
    Token advance() { return tokens_[position_++]; }

    bool check_symbol(std::string_view symbol) const {
        return peek().kind == TokenKind::symbol && peek().text == symbol;
    }

    bool match_symbol(std::string_view symbol) {
        if (!check_symbol(symbol)) {
            return false;
        }
        advance();
        return true;
    }

    void expect_symbol(std::string_view symbol) {
        if (!match_symbol(symbol)) {
            throw std::runtime_error("期望符号 " + std::string(symbol));
        }
    }

    void expect_arrow() {
        if (peek().kind != TokenKind::arrow) {
            throw std::runtime_error("教学版关系只支持从左到右的 -> 方向");
        }
        advance();
    }

    bool match_keyword(std::string_view keyword) {
        if (peek().kind != TokenKind::word || !equals_keyword(peek().text, keyword)) {
            return false;
        }
        advance();
        return true;
    }

    void expect_keyword(std::string_view keyword) {
        if (!match_keyword(keyword)) {
            throw std::runtime_error("期望关键字 " + std::string(keyword));
        }
    }

    std::string expect_word(std::string_view description) {
        if (peek().kind != TokenKind::word) {
            throw std::runtime_error("期望" + std::string(description));
        }
        return advance().text;
    }

    void finish() {
        if (match_symbol(";")) {
            // optional terminator
        }
        if (peek().kind != TokenKind::end) {
            throw std::runtime_error("命令末尾存在多余内容: " + peek().text);
        }
    }

    std::vector<Token> tokens_;
    std::size_t position_{0};
};

}  // namespace

ParseResult parse_cypher(const std::string& command) {
    try {
        Lexer lexer(command);
        auto tokens = lexer.scan();
        if (tokens.size() == 1) {
            return ParseResult{std::nullopt, "命令不能为空"};
        }
        return ParseResult{Parser(std::move(tokens)).parse(), {}};
    } catch (const std::exception& exception) {
        return ParseResult{std::nullopt, exception.what()};
    }
}

}  // namespace minidb::neo4j
